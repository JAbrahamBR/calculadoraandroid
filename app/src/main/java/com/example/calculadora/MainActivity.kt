...
class MainActivity : AppCompatActivity() {
    ...
    fun calcular(view : View){
        ...
        var concatenar=tResultado?.text.toString()+textBoton
        var concatenarSinCeros=quitarCerosIzquierda(concatenar)
        if (textBoton=="="){
            var resultado=0.0
            try {
                resultado=eval(tResultado?.text.toString())
                tResultado?.text=resultado.toString()
            }catch (e:Exception){
                tResultado?.text=e.toString()
            }

        }else if (textBoton=="BORRAR"){
            tResultado?.text="0"
        }else{
            tResultado?.text=concatenarSinCeros
        }
